...
public class Transaction {
    ...
    // given the index find the corresponding Input and its previous transaction's output,
    // based on the previous transaction output's hash, index, value, address
    ...
    public byte[] getRawDataToSign(int index) {
        ...
        // buffer receives exactly 4 bytes (1 int)
        ByteBuffer b = ByteBuffer.allocate(Integer.SIZE / 8);
        b.putInt(in.outputIndex);
        byte[] outputIndex = b.array();
        // sigData firstly contains the previous transaction's hash
        if (prevTxHash != null)
            for (int i = 0; i < prevTxHash.length; i++)
                sigData.add(prevTxHash[i]);
        // then it contains the output index
        ...
        for (Output op : outputs) {
            ...
            // then it contains the outputs' values
            for (int i = 0; i < value.length; i++)
                sigData.add(value[i]);
            // then it contains the outputs' addresses
            for (int i = 0; i < addressBytes.length; i++)
                sigData.add(addressBytes[i]);
        }
        // convert sigData to a byte array
    ...
    public byte[] getRawTx() {
        // convert this transaction into raw bytes
        ArrayList<Byte> rawTx = new ArrayList<Byte>();
        // sequentially convert the raw bytes of inputs
        for (Input in : inputs) {
            // the first component into bytes is the previous output's hash
            byte[] prevTxHash = in.prevTxHash;
            ByteBuffer b = ByteBuffer.allocate(Integer.SIZE / 8);
            b.putInt(in.outputIndex);
            byte[] outputIndex = b.array();
            ...
            // the second component into bytes is the index of the output
            for (int i = 0; i < outputIndex.length; i++)
